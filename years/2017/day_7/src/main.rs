...
pub struct TowerBlock {
    name:String,
    parent:Option<String>, //only ever one?
    childern:Vec<TowerBlock> //can be empty, just one, or many
}

fn parse_input (input: String) -> TowerBlock {
    let parent_vec: Vec<&str> = input.split(" (").collect();
    
    if parent_vec.len() < 1 {
        println!("Bad input! Tower did not include at least a parent and weight");
        std::process::exit(1);
    }
    
    let mut childern_vec: Vec<&str> = parent_vec[1].split("-> ").collect();
    
    if childern_vec.len() > 1 {
        childern_vec.remove(0); //remove "weight) ->" 
        childern_vec = childern_vec[0].split(", ").collect();
        //childern_vec.remove(0); //removing empty space 
    }
    else {
        //println!("no childern $$$");
    }
    
    let mut parent: TowerBlock = TowerBlock{name:parent_vec[0].to_string(), parent:None, 
        childern:Vec::new()};
        
    if childern_vec.len() > 1 { //has childern 
        for kid in childern_vec.iter() {
            //println!("adding childern");
            parent.childern.push(TowerBlock{
                name:kid.to_string(),
...
fn find_base(input: String) -> String {
    let lines:Vec<&str> = input.split("\n").collect();
    
    let mut tower_vec: Vec<TowerBlock> = Vec::new();
    
    for line in lines.iter() {
        tower_vec.push(parse_input(line.to_string()));
    }
    
    let mut tower_test = tower_vec.clone();

    for tower in tower_vec.iter_mut() {
        for child in tower.childern.iter_mut() {
            let child_match = &child.name;
            for test_match in tower_test.iter_mut() {
                if test_match.name == *child_match {
                    test_match.parent=Some(tower.name.to_string());
                }
            }
        }
    } 
    
    let mut master_parent = "out".to_string();

    for i in tower_test.iter() {
        match i.parent {
            Some(ref s) => (),
            None => master_parent = i.name.to_string()
        }
    }
...
fn main() {
    //simple test parent with childern
    //let test = "fwft (72) -> ktlj, cntj, xhth";
    //println!("TESTING: {:?}", parse_input(test.to_string()));
    
    //test just single entity
    //let test = "ktlj (57)";
    //println!("TESTING: {:?}", parse_input(test.to_string()));
...
cntj (57)";
    ...
    println!("{}", find_base(lines.to_string()));

    //challenge!
    let challenge_line = "jovejmr (40)
fesmk (24)
gwhfv (74)
...
gynzo (86)
wnwzo (37)";
println!("{}", find_base(challenge_line.to_string()));
}