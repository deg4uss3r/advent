...
fn compiler(input: String) -> isize {
    //hash of register and value
    let mut registers: HashMap<String, isize> = HashMap::new();
    let instructions: Vec<&str> = input.split("\n").collect();

    for ins in instructions.iter() {
        //split instructions
        //if register doesn't exist create it in the hash with 0 then move on
        //else parse instruction and check logic <,>,>=,<=,!=,==
        let i: Vec<&str> = ins.split(" ").collect();

            if !registers.contains_key(i[4]) {
                registers.insert(i[4].to_string(),0);
            } 

            let operator_value = registers[i[4]];
            let register_value = registers.entry(i[0].to_string()).or_insert(0);
            let logic_num = i[6].parse::<isize>().unwrap();
            let register_mod = i[2].parse::<isize>().unwrap();

            if i[5] == ">" {
                if operator_value >  logic_num {
                    if i[1] == "inc" {
                        *register_value+=register_mod;
                    }
                    else {
                       *register_value-=register_mod; 
                    }
                ...
                println!("unsupport logic type");
            }
    }

    let mut highest = <isize>::min_value();
...
c dec -10 if a >= 1
c inc -20 if c == 10";
    println!("{}", compiler(instructions.to_string()));

    //challenge
    let instructions = "t inc 245 if xq != 0
hi inc 119 if gf >= -5
w dec 529 if s == 0
p inc 19 if fi > -2
