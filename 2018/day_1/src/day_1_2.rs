...
fn compute_frequency(input: &str) -> i64 {
    let split_freqs = input.split("\n");
    let num_freqs: Vec<i64> = split_freqs
        .map(|x| x.parse::<i64>().expect(&format!("Error could not parse {} as i64", x)))
        .collect();
    ...
    while !frequencies.contains(&freq_sum) {
        ...
        if freq_index >= num_freqs.len() { //loop around the frequencies if we made it to the end of the list
            freq_index = 0;
        }
...
-1
-110292";
    println!("{}", compute_frequency(puzzle_input));
}
